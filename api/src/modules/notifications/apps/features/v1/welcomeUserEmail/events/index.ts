...
import { bullMqRedisConnection, Container, GuardWrapper, JsonString, ReplyMessageBullMq, RequestReplyConsumerBullMq, StatusCodes, WorkerBullMq } from "@kishornaik/utils";
...
export const subscribeWelcomeUserEmailIntegrationEvent:WorkerBullMq=async()=>{
  ...
  const worker=await consumer.startConsumingAsync<JsonString,JsonString>(queueName,async (reply)=>{
    ...
    logger.info(`WelcomeUserEmailIntegrationEvent Job started: traceId: ${traceId} | correlationId: ${correlationId} | jobId: ${reply.id}`);
    ...
    // validation
    const validationResult=await Container.get(WelcomeUserEmailValidationService).handleAsync({
      dto:mapResult.value,
    ...
    const emailSentResult=await Container.get(EmailSentService).handleAsync(mapResult.value);
  ...
  worker.on('completed', (job) => {
		logger.info(`WelcomeUserEmailIntegrationEvent Job completed: traceId: ${job.data.traceId} | correlationId: ${job.data.correlationId} | jobId: ${job.id}`);
	});

	worker.on('failed', (job, err) => {
    logger.error(`WelcomeUserEmailIntegrationEvent Job failed: traceId: ${job.data.traceId} | correlationId: ${job.data.correlationId} | jobId: ${job.id} | error: ${err.message}`);
	});
}
