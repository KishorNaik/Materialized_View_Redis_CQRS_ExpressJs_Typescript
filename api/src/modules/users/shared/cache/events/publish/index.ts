import { bullMqRedisConnection, ExceptionsWrapper, GuardWrapper, IServiceHandlerVoidAsync, JsonString, RequestReplyProducerBullMq, Result, ResultError, ResultFactory, sealed, SenderReceiverProducerBullMq, SendReceiverMessageBullMq, Service, StatusEnum, VOID_RESULT, VoidResult } from "@kishornaik/utils";
...
export interface IPublishUserSharedCacheDomainEventService extends IServiceHandlerVoidAsync<IPublishUserSharedCacheDomainEventServiceParameters>{}

@sealed
@Service()
export class PublishUserSharedCacheDomainEventService implements IPublishUserSharedCacheDomainEventService{
  public handleAsync(params: IPublishUserSharedCacheDomainEventServiceParameters): Promise<Result<VoidResult, ResultError>> {
    return ExceptionsWrapper.tryCatchResultAsync(async ()=>{
      ...
      await producer.sendAsync<JsonString>(
				`JOB:${queueName}`,
				messages
			);
