...
Container.set<UpdateUserSettingsDbService>(
	UpdateUserSettingsDbService,
	new UpdateUserSettingsDbService()
);
...
export interface IUpdateEmailService
	extends IServiceHandlerVoidAsync<IUpdateEmailServiceParameters> {}
...
export class UpdateEmailService implements IUpdateEmailService {
	...
	public handleAsync(
		params: IUpdateEmailServiceParameters
		...
		return ExceptionsWrapper.tryCatchResultAsync(async () => {
			...
			const updateResult = await this._updateUserSettingsDbService.handleAsync(
				userSettingsEntity,
				queryRunner
			);
			...
			if (updateRowVersionResult.isErr())
				return ResultFactory.error(
					updateRowVersionResult.error.statusCode,
